...
def main():
    ...
    pat = args.outputs_glob
    # Path.glob only supports *relative* patterns; handle absolute via glob(..., recursive=True)
    if os.path.isabs(pat) or (":" in pat) or pat.startswith(("\\", "/")):
        outputs = [Path(p) for p in glob(pat, recursive=True)]
    else:
        outputs = sorted(Path().glob(pat))
