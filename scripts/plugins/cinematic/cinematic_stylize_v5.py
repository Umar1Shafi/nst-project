...
def run(
    ...
    _dtype = torch_dtype if torch_dtype is not None else (torch.float16 if "cuda" in device else torch.float32)
...
def main():
    ...
    p.add_argument("--dtype", choices=["auto", "fp16", "fp32"], default="auto", help="Force precision; use fp32 if your GPU trips cuBLAS.")
    ...
    if args.dtype == "fp32":
        dtype = torch.float32
    elif args.dtype == "fp16" and device == "cuda":
        dtype = torch.float16
    else:
    ...
    # --- choose annotator + controlnet BEFORE building any pipeline ---
    if args.subject == "portrait":
        ...
        cimg = control_image_softedge(src)
    else:
        ...
        cimg = control_image_depth(src)

    if cimg is None:
        print("[warn] Annotator missing — using RGB image as weak control.")
        cimg = src

    # --- render using your existing run(...) which builds the pipe correctly ---
    out = run(
        model_id=args.model,
        device=device,
        src=src,
        ...
        seed=args.seed,
        torch_dtype=dtype,     # pass dtype through
        low_vram=args.low_vram # pass low-vram through if your run() supports it
    )
