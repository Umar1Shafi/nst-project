...
# -*- coding: utf-8 -*-
"""
Phase 3 Advanced Eval (images-only):
- Inputs: 8 stylized outputs + 2 reference inputs (portrait, scene)
- Outputs: metrics CSV + per-image sidecar JSONs (no reruns of stylizers)

Metrics:
  ArcFace cosine (portraits only; identity)      -> higher is better
  LPIPS (all images; perceptual distance)        -> lower is better
  PSNR, SSIM (divergence vs input)               -> higher is better
  Edge-IoU (Sobel edges overlap; structure)      -> higher is better

Author: You (NST Phase 3)
"""
...
# Optional deps – we load lazily and fail gracefully
def _try_imports():
    mods = {}
    try:
    ...
    try:
        ...
        mods["torch"] = None
    try:
        import lpips  # needs torch
        mods["lpips"] = lpips
    except Exception:
        mods["lpips"] = None
    # ArcFace (insightface uses onnxruntime; CPU works)
    try:
        import insightface
...
def ssim_metric(a_gray: np.ndarray, b_gray: np.ndarray) -> Optional[float]:
    if MODS["ssim"] is None:
        return None
    try:
        return float(MODS["ssim"](a_gray, b_gray, data_range=255.0))
    except Exception:
        return None

def sobel_edges(gray: np.ndarray) -> np.ndarray:
    # Fast Sobel via numpy gradients
...
def lpips_score(a_rgb: np.ndarray, b_rgb: np.ndarray) -> Optional[float]:
    if MODS["torch"] is None or MODS["lpips"] is None:
        return None
...
# ArcFace cosine on portraits (CPU ok via onnxruntime)
_ARC_APP = None
def _load_arcface():
    global _ARC_APP
    if _ARC_APP is not None: return _ARC_APP
    if MODS["FaceAnalysis"] is None:
        return None
...
def arcface_cosine(a_rgb: np.ndarray, b_rgb: np.ndarray) -> Optional[float]:
    ...
    if not fa or not fb:
        return None
    va = fa[0].normed_embedding
    vb = fb[0].normed_embedding
...
def main():
    ...
    p.add_argument("--outputs-glob", required=True,
                   help="Glob for stylized outputs (e.g. out/phase3/**/*.png)")
    p.add_argument("--portrait-input", required=True,
                   help="Reference portrait input (e.g. data/content/portrait2.jpg)")
    p.add_argument("--scene-input", required=True,
                   help="Reference scene input (e.g. data/content/street.jpg)")
    p.add_argument("--outdir", default="report/phase3_eval_advanced",
                   help="Where to save CSV and sidecars")
    args = p.parse_args()

    outdir = Path(args.outdir); outdir.mkdir(parents=True, exist_ok=True)

    outputs = sorted([Path(p) for p in Path().glob(args.outputs_glob)])
    if not outputs:
        print(f"[error] No outputs matched: {args.outputs_glob}")
        sys.exit(2)

    portrait_ref = Path(args.portrait_input); scene_ref = Path(args.scene_input)
    if not portrait_ref.exists() or not scene_ref.exists():
        print("[error] One or both reference inputs do not exist.")
        print(f"portrait: {portrait_ref} | scene: {scene_ref}")
        sys.exit(3)
    ...
    # Friendly summary
    def _fmt(x): return "n/a" if x is None else str(x)
    portraits = [r for r in rows if r["subject_guess"] == "portrait"]
    scenes = [r for r in rows if r["subject_guess"] == "scene"]
    print("\n=== Phase 3 Advanced Eval — Summary ===")
    print(f"Outputs evaluated: {len(rows)}  |  CSV: {csv_path}")
    if portraits:
        af_vals = [r["arcface_cos"] for r in portraits if r["arcface_cos"] is not None]
        if af_vals:
            print(f"Portrait identity (ArcFace cosine): mean={sum(af_vals)/len(af_vals):.3f} "
                  f"min={min(af_vals):.3f} max={max(af_vals)::.3f}")
        else:
            print("Portrait identity (ArcFace): n/a (face model missing or faces not detected)")
    lp_vals = [r["lpips"] for r in rows if r["lpips"] is not None]
    if lp_vals:
        print(f"LPIPS (all): mean={sum(lp_vals)/len(lp_vals):.3f} "
              f"min={min(lp_vals):.3f} max={max(lp_vals):.3f}")
    else:
        print("LPIPS: n/a (lpips/torch missing)")

    print("=======================================\n")
    print("Tip for report: ")
    print("• ArcFace ≥ 0.85 on portraits indicates strong identity preservation.")
    print("• LPIPS ≤ 0.30 suggests moderate stylization while retaining content.")
    print("• Edge-IoU complements SSIM to show structural fidelity on scenes.")
